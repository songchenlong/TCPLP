...
from tcplp_longformer import TcplpTokenizer, TcplpConfig, TcplpModel, TcplpForFinetune
# from tcplp_bert import TcplpTokenizer, TcplpConfig, TcplpModel, TcplpForFinetune
from collator import FinetuneDataCollatorWithPadding, EvalDataCollatorWithPadding

from finetune_dataloader import TcplpTrainDataset, TcplpEvalDataset
...
def load_data(args):
    ...
    item_meta_dict_filted, item2id, id2item = dict(), dict(), dict()

    for k, v in item_meta_dict.items():
        if k not in item2id.keys():
            item2id[k] = len(item2id.keys())
            id2item[item2id[k]] = k
...
def encode_all_items(model: TcplpModel, tokenizer: TcplpTokenizer, tokenized_items, args):
    model.eval()
...
def main():
    ...
    parser.add_argument('--model_name_or_path', type=str,
                        default='/data/SongChenLong/model/longformer-base-4096/')
    ...
    train_d, val_d, test_d, item_meta_dict, item2id, id2item = load_data(args)

    config = TcplpConfig.from_pretrained(args.model_name_or_path)
    config.max_item_embeddings = 51
    config.attention_window = [64] * 12
    config.max_token_num = 1024
    config.item_num = len(item2id.keys())
    ...
    path_corpus = Path(args.meta_file)
    ...
    tokenized_items = {item2id[item_id]: input_ids for item_id, input_ids in doc_tuples}
    pool.close()
    pool.join()


    finetune_data_collator = FinetuneDataCollatorWithPadding(tokenizer, tokenized_items, item2id)
    eval_data_collator = EvalDataCollatorWithPadding(tokenizer, tokenized_items, item2id)

    train_data = TcplpTrainDataset(train_d, collator=finetune_data_collator)
    val_data = TcplpEvalDataset(val_d, collator=eval_data_collator)
    test_data = TcplpEvalDataset(test_d, collator=eval_data_collator)
    ...
    for name, param in model.named_parameters():
        if name.startswith('share_prompts'):
            param.requires_grad = False
    ...
    for frozen_name in frozen_params:
        print(frozen_name)
    print('trainable_params:')
    for train_name in trainable_params:
        print(train_name)
    ...
    for epoch in range(args.num_train_epochs):
        ...
        if (epoch + 1) % args.verbose == 0:
            test_metrics = eval(model, test_loader, args)
            print(f'Epoch: {epoch + 1}. Test set: {test_metrics}')


            if test_metrics['NDCG@10'] > best_target:
                print('Save the best model.')
                best_target = test_metrics['NDCG@10']
                patient = 8
